...
void loop() {
   while(Serial.available()>0) {    // make an array
      dataIn = Serial.read();
      step_size[count] = dataIn;
      count++;                                // count is the length of the array
      if (step_size[0] == 45){                // case you recieve a negative, defaults to false (positive)
         neg = true;
   ...
   if (step_size[0] >0){                     // must do this outside the loop because calculations are dependent on the size of the array
      for(i=0; i < count; i++){
          num_steps = num_steps + pow(10,(count-1))*(step_size[i]-'0');    //convert it to a base 10 number for us humans 
          step_size[i] = 0;                                                // clear out the memory for the next round
          count--;                                                         
